 ...
 * `lookup_weather` tool and asked to either request locations or produce a final answer.
 *
 * Requires OPENAI_API_KEY. The `openai` package is imported dynamically so the rest of the
 * harness — and the offline test suite — never need it installed.
 */
export class OpenAiLlmClient implements LlmClient {
  ...
  async decide(goal: string, gathered: WeatherResult[]): Promise<LlmDecision> {
    ...
    const gatheredText = gathered.length
      ? gathered.map((g) => `${g.location}: ${g.temperatureC}°C, ${g.summary}, wind ${g.windKph} kph`).join("\n")
      : "(none yet)";
