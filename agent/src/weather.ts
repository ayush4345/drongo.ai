...
/**
 * A real metered weather service backed by the keyless Open-Meteo API: it geocodes the
 * place name, then fetches current conditions. One unit per call.
 *
 * The HTTP client is injected so tests run offline with canned responses; production uses
 * {@link FetchHttpClient}. This is what makes the provider agent "meaningful" — it sells
 * genuine, useful work, metered and paid per call through the channel.
 */
