...
/**
 * An LLM-driven consumer agent. Given a natural-language goal, it asks the LLM which
 * locations to look up, buys each weather call through the metered channel (paying per
 * call), feeds the results back to the LLM, and repeats until the LLM produces a final
 * answer. The channel handles the privacy-preserving payment; the LLM handles the
 * "what to ask" — together they are a meaningful consumer that genuinely uses the
 * provider's service.
 */
