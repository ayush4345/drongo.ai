...
function open(crypto: ShadowCrypto, unitsPerCall = 1n, rate = 2_000n, escrow = 20n * USDC) {
...
test("a paid call is served, returns a result, and advances the bill", async () => {
  ...
  const out = channel.call({ prompt: "hello" });
...
test("settlement equals total served units * rate", async () => {
  const crypto = await ShadowCrypto.build();
  const { channel } = open(crypto);
  for (let i = 0; i < 10; i++) assert.equal(channel.call({ prompt: `r${i}` }).served, true);
...
test("provider prices independently and refuses an underpaid voucher", async () => {
  ...
  const resp = channel.provider.serve({ request: { prompt: "x" }, cost: 1n, voucher });
...
test("provider halts at the escrow ceiling and serves nothing further", async () => {
  ...
  assert.equal(channel.call({ prompt: "a" }).served, true);
  assert.equal(channel.call({ prompt: "b" }).served, true);
  assert.equal(channel.call({ prompt: "c" }).served, true);

  const over = channel.call({ prompt: "d" });
...
test("the service only runs when paid (no result on refusal)", async () => {
  ...
  const resp = channel.provider.serve({ request: { prompt: "x" }, cost: 1n, voucher: forged });
  assert.equal(resp.served, false);
